...
def get_energy(time, device, verbose=False):
    ...
    try:
        ...
        # NVIDIA GPU
        elif device == torch.device("gpu") or device == torch.device("cuda"):
            import pynvml  # included in python 3.9

            pynvml.nvmlInit()
            # Returns device handle, index 0 specifies the first GPU
            handle = pynvml.nvmlDeviceGetHandleByIndex(0)
            # Retrieves power usage for GPU in milliwatts
            ...
            with open('/sys/class/powercap/intel-rapl/intel-rapl:0/energy_uj', 'r') as file:
                # Gets energy in microjoules, then converts to joules
                energy = int(file.read().strip()) / 1_000_000
                if verbose:
