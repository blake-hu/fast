    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            if X_val is not None and y_val is not None:
                validation_loss, validation_f1 = self._validate(X_val, y_val)
                print(
                    f"Epoch {epoch+1}/{self.num_epochs} | Training Loss : {average_loss} | Validation Loss : {validation_loss}")
                if self.stopper.early_stop(validation_loss):
                    break
            else:
                print(
                    f"Epoch {epoch+1}/{self.num_epochs} | Training Loss : {average_loss}")
