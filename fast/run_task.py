...
def compute_single_embeddings(model, tokenizer, loader, device):
    embeddings = []
    model.eval()
    with torch.no_grad():
        for text_batch in tqdm(loader):
            inputs = tokenize_batch(tokenizer, text_batch)
...
def compute_pair_embeddings(model, tokenizer, loader, device):
    embeddings = []
    model.eval()
    with torch.no_grad():
        for text_batch1, text_batch2 in tqdm(loader):
            inputs1 = tokenize_batch(tokenizer, text_batch1)
            inputs2 = tokenize_batch(tokenizer, text_batch2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', required=True, help='Model name to generate embeddings')
    parser.add_argument('--task', required=True, help='GLUE task')
    parser.add_argument('--embedding', required=True, help='Embedding type')
    parser.add_argument('--device', required=False, help='Optional, can specific specific device for model to run')
    ...
    if all(path.exists() for path in paths):
        ...
        if task_config.sentence_type == "one":
            train_embed = compute_single_embeddings(model, tokenizer, train_loader, device)
            val_embed = compute_single_embeddings(model, tokenizer, val_loader, device)
            test_embed = compute_single_embeddings(model, tokenizer, test_loader, device)
        elif task_config.sentence_type == "two":
            train_embed = compute_pair_embeddings(model, tokenizer, train_loader, device)
            val_embed = compute_pair_embeddings(model, tokenizer, val_loader, device)
            test_embed = compute_pair_embeddings(model, tokenizer, test_loader, device) 
